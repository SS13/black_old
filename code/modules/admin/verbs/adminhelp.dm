...
*/
	...
	//The symbol ՠ(fancy multiplication sign) will be used to mark where to put replacements, so the original message must not contain it.
	...
	var/list/replacement_value = list()		//When a word match is found, the word matched will get replaced with an ՠ(fancy multiplication symbol).
											//This list will contain a list of values which the ՠwill be replaced with in the same order as indexes in this list.
