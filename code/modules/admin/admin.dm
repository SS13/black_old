...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if (href_list["CentcommReply"])
		...
		log_admin("[src.owner] replied to [key_name(M)]'s Centcomm message with the message [sanitize(html_decode(input))].")
		message_admins("[src.owner] replied to [key_name(M)]'s Centcomm message with the message [sanitize(html_decode(input))].")
		M << "You hear something crackle in your headset for a moment before a voice speaks. \"Please stand by for a message from Central Command. Message as follows. [sanitize(html_decode(input))] Message ends.\""
	...
	if (href_list["SyndicateReply"])
		...
		src.owner << "You sent [sanitize(html_decode(input))] to [M] via a secure channel."
		log_admin("[src.owner] replied to [key_name(M)]'s Syndicate message with the message [sanitize(html_decode(input))].")
		message_admins("[src.owner] replied to [key_name(M)]'s Syndicate message with the message [sanitize(html_decode(input))].")
		M << "You hear something crackle in your headset for a moment before a voice speaks. \"Please stand by for a message from your benefactor. Message as follows, agent. [sanitize(html_decode(input))] Message ends.\""
...
/obj/admins/proc/show_player_info(var/key as text)
	...
	if(!infos)
		...
		for(var/datum/player_info/I in infos)
			...
			if(I.author == usr.key || usr.client.holder.rank == "Game Master")
