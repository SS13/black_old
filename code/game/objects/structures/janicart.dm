/obj/structure/stool/bed/chair/janicart
	...
	density = 1
	flags = OPENCONTAINER
	var/horse = 0 //for no need in fueld --Lethal Ghost
	//copypaste sorry
...
/obj/structure/stool/bed/chair/janicart/examine()
	...
	if(!horse)
		...
		usr << "\icon[src] This is knights's horse"
	...
	if(horse)
		if (istype(W, /obj/item/weapon/reagent_containers/food/snacks/grown/wheat))
			for (var/mob/O in viewers(src, null))
				O.show_message("<B>[src]</B> Om-Nom-Nom")
				user.drop_item()
				del(W)
		return
...
/obj/structure/stool/bed/chair/janicart/relaymove(mob/user, direction)
	if(user.stat || user.stunned || user.weakened || user.paralysis)
		unbuckle()
	if(istype(user.l_hand, /obj/item/key) || istype(user.r_hand, /obj/item/key) || horse)
		step(src, direction)
		update_mob()
		handle_rotation()
	else
...
/obj/structure/stool/bed/chair/janicart/horse
	horse = 1
	name = "Horse"
